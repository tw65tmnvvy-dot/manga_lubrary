@echo off
REM Improved Windows installer script for non-technical users.
REM This script checks for Python 3.8+, creates a venv, installs requirements (including Pillow),
REM and checks for sqlite3 support. If Python is missing or too old it will show a link and stop.

SETLOCAL EnableDelayedExpansion
SET VENV_NAME=manga-env

necho Checking for Python (3.8+)...
...
REM Check for sqlite3 support (informational)
echo Checking sqlite3 support in Python...
...
echo If you want a standalone executable later, run (inside the venv):
